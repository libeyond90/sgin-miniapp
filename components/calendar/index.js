import {Calendar,toLunar} from "./calendar.js"
...
Component({
    ...
    methods: {
      getToday(){
        const evt = {};
        evt.detail = { value: getToday().join("-") };
        this.change(evt)
      },
      getLunar(){
        if(this.data.value){
          const dates = this.data.value.split('-');
          // let date = new Date(this.data.value);
          let selectLunar = toLunar(dates[0],dates[1],dates[2])
          var d = Lunar.fromDate(new Date(dates[0],dates[1]-1,dates[2]));
          // const gz = d.getTimeInGanZhi();
          selectLunar.yi = d.getDayYi();
          selectLunar.ji = d.getDayJi();
          selectLunar.label = selectLunar.festival || selectLunar.solar || selectLunar.Term
          this.setData({
            selectLunar
          })
          // console.log(selectLunar)
        }
      },
      //初始日历
      initCalendar(){
        ...
        this.setData({
          ...
          currentDate: "", // 标记今天
          animate: "", // 切换周月视图时开启动画过渡
          value:this.data.value?this.data.value:getToday().join("-")
        },()=>{
          // console.log('init',this.data.value)
          this.getLunar();
        })
      },
      //初始视图
      ...
      opViewType(){   
        this.setData({
          viewType: this.data.viewType == 'month'?'week':'month'
        },()=>{
          this.initCalendar()
          this.initView()
        })
